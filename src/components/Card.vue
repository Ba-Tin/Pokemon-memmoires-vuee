<template>
  <div class="card" :class="{ disabled: isDisabled }" :style="{
    ...
    width: `${(((750 - 16 * 4) / Math.sqrt(cardsContext.length) - 16) * 3) / 4}px`,
    perspective: `${((((750 - 16 * 4) / Math.sqrt(cardsContext.length) - 16) * 3) / 4) * 2}px`
  }">
    <div class="card__inner" :class="{ 'is-flipped': isFlipped }" @click="onToogleFlipCard">
